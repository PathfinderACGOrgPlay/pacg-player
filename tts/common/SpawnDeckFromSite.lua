...
function addDeck(systemId, deckInfo, results)
    local cards = {}
    for i, v in ipairs(deckInfo.cards) do
        ...
        for j, w in ipairs(v.info) do
            ...
            for k = 1, w.count, 1 do
                print(w.name .. "_" .. j .. "_" .. x .. "_" .. y)
                table.insert(cards, Decker.Card(asset, y, x, {
                    name = w.name
                }))
            end
        end
    ...
    table.insert(results, deck.data)
end
        ...
        if (result.is_error) then
            ...
            if (data.error) then
                ...
                if (data.cards.one.deck.name) then
                    addDeck(data.characterData.systemId, data.cards.one, results)
                end

                if (data.cards.two.deck.name) then
                    addDeck(data.characterData.systemId, data.cards.two, results)
                end

                if (data.cards.three.deck.name) then
                    addDeck(data.characterData.systemId, data.cards.three, results)
                end
